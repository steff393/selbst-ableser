...
// TestParseRemovableMountsKeepsMountOrder matters because AutoDetect
// takes the first medium when several are connected, rather than
// refusing to pick (see removable.go): "first" only means anything if
// the order here is /proc/mounts order.
func TestParseRemovableMountsKeepsMountOrder(t *testing.T) {
	mounts := strings.NewReader(strings.Join([]string{
		"/dev/sda1 /media/usb vfat rw 0 0",
		"/dev/sdb1 /media/usb2 vfat rw 0 0",
	}, "\n") + "\n")

	got, err := parseRemovableMounts(mounts, func(string) bool { return true })
	if err != nil {
		t.Fatalf("parseRemovableMounts: %v", err)
	}
	if len(got) != 2 || got[0] != "/media/usb" || got[1] != "/media/usb2" {
		t.Fatalf("got %v, want both media in mount order", got)
	}
}
